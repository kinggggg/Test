package com.zeek.concurrent.shengsiyuan.concurrency.concurrency5;
 ...
 * 2. 当调用await方法时，底层会先检查计数器是否已经归零，如果是的话，那么就首先执行可选的Runnable，接下来开始想一下generation
 * 3. 在下一个分代中，将会重置count的值为parties，并且创建新的Generation实例
 * 4. 同事会调用Condition的signalAll方法，唤醒所有在屏障前面等待的线程，让其开始执行
